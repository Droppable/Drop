  ...
  */
  ...
  this.createDOM = function (selector, options) {
    ...
    var _template = function (options) {
      ...
      var _showClose = options.showClose == true ? '<span class="drop-close ' + _showClose + '" data-drop-close>X</span>' : '';
      var _render = typeof options.render !== undefined && options.render.length > 0 ? 'data-drop-render="' + options.render + '"' : '';

      var _output = '<div class="drop" data-drop id="' + selector + '" ' + _onHover + ' ' + _render + ' ' + _onMouseup + '>';
      _output += '<button class="drop-button" data-drop-button>' + options.triggerText + '</button>';
      _output += '<div class="drop-container" data-drop-container>';
  ...
  this.insertDataRow = function (selector) {
    ...
    if (typeof selector !== undefined && !_self.isRegistered(selector)) {
      ...
      var _dropContainer = _drop.find('[data-drop-container]').first();
  ...
  this.hasValidDOM = function(selector) {
    ...
    if (typeof selector !== 'undefined' && typeof selector === 'string') {
      var _attribute = {
        'drop' : '[data-drop]',
        'dropId' : '[data-drop-id]',
        'dropTrigger' : '[data-drop-button]',
        'dropContainer' : '[data-drop-container]',
        'dropClose' : '[data-drop-close]'
      };
  ...
  this.isValidDom = function (selector) {
    ...
    if (typeof selector !== undefined && typeof selector == 'string') {
      ...
      if (_drop && _drop.length && typeof _drop !== undefined) {
        ...
        var _hasAttribute = _drop.filter('[data-drop]').length ? true : false;
        if (_hasAttribute || _hasClass) {
          return true;
        } else {
          return false;
  ...
  this.children = function (selector) {
    ...
    if (typeof selector != 'undefined' && _self.isValidDom(selector)) {
      ...
      var _dropTrigger = _drop.find('[data-drop-button]'),
          _dropClose,
          _dropContainer,
