var Drop = function () {
    ...
    // Version
    Last updated on 12th April 2017 @ 15:30
    ...
    */
    ...
    this.log = function (text) {
        return console.log('%c' + text, 'background: #000; color:#fff; padding: 4px;');
    };
    ...
    this.createDOM = function (selector, options) {
        ...
        var _template = function (options) {
            ...
            var _showClose = options.showClose == true ? '<button class="drop-close button button-icon' + _showClose + '"><em class="icon"><i class="fi-cross"></i></em></button>' : '';
            var _render = /*typeof options.render !== undefined*/typeof options.render != 'undefined' && options.render.length > 0 ? 'data-drop-render="' + options.render + '"' : '';
        ...
        if (/*typeof selector !== undefined*/typeof selector != 'undefined' && typeof selector == 'string') {
    ...
    this.insertDataRow = function (selector) {
        ...
        if (typeof selector != 'undefined' && _self.isRegistered(selector) == false) {
            ...
            if (typeof _dropId != 'undefined' && typeof _dropId != 'object' && typeof _dropId == 'string') {
                ...
                //console.log('Drop: Register ' + _dropId);
        ...
        // Row already exists
            ...
            // Let's try registering this one too with a different data-drop-id
            var _drop = _self.DOM(selector);
            var _dropId = _self.GUID(selector);
            var _dropContainer = _drop.find('.drop-container').first();

            if (typeof _dropId != 'undefined' && typeof _dropId != 'object' && typeof _dropId == 'string') {

                console.log('Drop: Register ' + _dropId + ' too.');
            ...
            // There is a problem with registering this drop
            else {
    ...
    this.renderDOM = function (selector) {
        ...
        if (/*typeof selector !== undefined*/typeof selector != 'undefined' && typeof selector == 'string') {
            ...
            if (_self.isRendered(selector)) {
                ...
                $('window, body, div').on('load resize scroll', function () {
    ...
    this.DOM = function (selector) {
        ...
        if (typeof selector !== 'undefined') {
            ...
            if (!_drop.length) {
                //console.log('Drop: Try finding ' + selector +' by looking up its data-drop-id.')
                _drop = $('.drop').filter('[data-drop-id="' + selector + '"]');
            }
        }
    ...
    this.getDataRow = function (selector) {
        ...
        if (typeof selector !== 'undefined' && _drop !== 'undefined') {
            ...
            //var _dropId = _drop.filter('[data-drop-id]').length ? _drop.data('drop-id') : undefined;
            var _dropId = _drop.filter('[data-drop-id]').length ? selector : undefined;
            var _data;
            var _data2 = [];

            // Loop through data array object
            for (var i = 0; i < _self.data.length; i++) {
                if (_self.data[i].id == _dropId && typeof _dropId != 'undefined' && /*typeof _dropId !== undefined*/typeof _dropId != 'undefined') {
                    _data = _self.data[i];
                }
            }
            // console.log(_data)
    ...
    this.onChange = function (handler, selector) {
        ...
        if (typeof handler != 'undefined' && typeof selector != 'undefined' && _self.isValidDom(selector)) {
            ...
            // So if the was a problem with one of the drops then not all stops working
            if (typeof _selector != 'undefined' && typeof _selector !== 'undefined' && typeof _data != 'undefined' && _data.id != '' && typeof _data.id != 'undefined') {

                // onHover
                if (_selector.drop.filter('[data-drop-onhover]').length) {
                    if (!_selector.drop.filter('[data-drop-eventhover]').length) {
                        _selector.drop.on('hover.dropHover', function () {
                            _self.toggleDom(_data, selector);
                        });
                        // To prevent attaching same event multiple times
                        _selector.drop.attr('data-drop-eventhover', '');
                    }
                }

                // onClick
                else {
                    if (!_selector.drop.filter('[data-drop-eventclick]').length) {
                        _selector.dropTrigger.on('click.dropClick', function () {
                            _self.toggleDom(_data, selector);
                        });
                        // To prevent attaching same event multiple times
                        _selector.drop.attr('data-drop-eventclick', '');
                    }
                }

                // onClose
                if (_selector.dropClose.length) {
                    if (!_selector.drop.filter('[data-drop-eventclose]').length) {
                        _selector.dropClose.on('click.dropClose', function () {
                            _self.toggleDom(_data, selector);
                        });
                        // To prevent attaching same event multiple times
                        _selector.drop.attr('data-drop-eventclose', '');
                    }
                }
            }
        }
...
Drop.prototype.init = function () {
    ...
    // Let's register available drops in the DOM
    _self.register();
...
Drop.prototype.open = function (selector) {
    ...
    // Little tweak to support DOM objects e.g. $('#drop')
    if (typeof selector != 'undefined' && typeof selector == 'object') {
        selector = selector.data('drop-id');
...
Drop.prototype.register = function (selector, options) {
    ...
    //console.log(typeof options, typeof selector)
    ...
    // Register drops with specified selector and options only
    if (typeof selector != 'undefined' && typeof selector == 'string') {
        ...
        // Register Drop through the markup in the DOM
            ...
            // The selector must be a valid Drop DOM
            ...
            if (_self.isValidDom(_selector) && !_self.isRegistered(_selector)) {
                ...
                //console.log('Re-register ' + _selector)

                var _data = _self.getDataRow(_selector);

                //console.log(_data)
                if (typeof _data != 'undefined' && _data.enable ==  0) {
                    _data.enable = 1;
                } else {
                    console.warn('Drop: This drop has already been registered.');
                }
            }
                // If none of above is true
            else {
                console.info('Drop: This drop is either registered or invalid.');
            }
        }
    }

    // Register all the drops found in the DOM
        ...
        var _selector = _self.selector.not('[data-drop-id]');
        ...
        if (_selector.length) {
            for (var i = 0; i < _selector.length; i++) {
                ...
                // Attach event handlers to this newly registered drop
                var _data = _self.getDataRow(_dropId);
        ...
        /*
        var _unregisteredSelector = _self.selector.not('[data-drop-id]');

        //console.log(_unregisteredSelector.find('> .drop-container').length)

        // If any unregistered selector exists then continue on
        if (_self.selector.not('[data-drop-id]').length) {

            //_self.log('Drop: Register ' + _self.selector.not('[data-drop-id]').length + ' drops found in the DOM.');

            // Loop through unregistered drops
            for (var i = 0; i < _unregisteredSelector.length; i++) {

                var _drop = $(_unregisteredSelector[i]);

                //console.log(_drop)

                //if (_unregisteredSelector.find('> .drop-container'))

                    _self.log('Drop: Register ' + _self.selector.not('[data-drop-id]').length + ' drops found in the DOM.');

                // Create data row from DOM
                var _insertData = _self.insertDataRow(_drop);

                if (_insertData) {

                    // Get the drop id from DOM
                ...
                // Error create data row
                else {
                    console.error('Drop: Error creating data row for ' + _dropId + '.');
                }
            }
        }
        */
        ...
        console.warn('Drop: No new drop found.');
    }
...
Drop.prototype.unregister = function (selector) {
    ...
    if (/*typeof selector !== undefined*/typeof selector != 'undefined' && _self.isValidDom(selector)) {
